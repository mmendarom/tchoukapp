  ...
  it('includes score, period score, own points, opponent own points and core stats', () => {
    const report = buildMatchReportData(
      match([
        ...
        point({ id: 'u-2', scoringTeam: 'uruguay', playerId: 'p1' }),
        point({ id: 'u-3', scoringTeam: 'uruguay', playerId: 'p2', periodNumber: 2, clock: { period: 2, secondsElapsed: 12 } }),
        ...
        point({ id: 'rd-player-1', kind: 'opponent_defense', team: 'opponent', playerId: 'p1', defenseLocation: { x: 0.5, y: 0.3 } } as Partial<MatchEvent>),
        point({ id: 'rd-player-2', kind: 'opponent_defense', team: 'opponent', playerId: 'p2', periodNumber: 2, clock: { period: 2, secondsElapsed: 18 }, defenseLocation: { x: 0.6, y: 0.3 } } as Partial<MatchEvent>),
    ...
    expect(report.finalScore).toEqual({ uruguay: 4, opponent: 2 });
    expect(report.scoreByPeriod[0].score).toEqual({ uruguay: 3, opponent: 2 });
    ...
    expect(report.totals.topScorers).toEqual([{ label: '#1 Mauro', total: 2 }, { label: '#2 Marcelo', total: 1 }]);
    expect(report.totals.defenses).toEqual([{ label: '#1 Mauro', total: 1 }]);
    expect(report.totals.opponentDefenses).toBe(4);
    ...
    expect(report.periods[0].maps.uruguayPoints).toEqual([{ x: 0.75, y: 0.3 }, { x: 0.75, y: 0.3 }]);
    ...
    expect(report.totalMaps.uruguayPoints).toEqual([{ x: 0.75, y: 0.3 }, { x: 0.75, y: 0.3 }, { x: 0.75, y: 0.3 }]);
    expect(report.totalMaps.opponentPoints).toEqual([{ x: 0.25, y: 0.4 }]);
    expect(report.totalMaps.opponentDefenses).toHaveLength(4);
  });
